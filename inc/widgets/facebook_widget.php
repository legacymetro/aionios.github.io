...
class solopine_facebook_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_facebook_widget', 'description' => __('A widget that displays a Facebook Like Box', 'solopine') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_facebook_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_facebook_widget', __('Rosemary: Facebook Like Box', 'solopine'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		?>
			...
			<iframe src="//www.facebook.com/plugins/likebox.php?href=<?php echo esc_url($page_url); ?>&amp;width=<?php echo esc_html($width); ?>&amp;colorscheme=light&amp;show_faces=<?php if($faces) { echo 'true'; } else { echo 'false'; } ?>&amp;border_color&amp;stream=<?php if($stream) { echo 'true'; } else { echo 'false'; } ?>&amp;header=<?php if($header) { echo 'true'; } else { echo 'false'; } ?>&amp;height=<?php echo esc_html($height); ?>&amp;show_border=false" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:<?php echo esc_html($width); ?>px; height:<?php echo esc_html($height); ?>px; background:#fff;" allowTransparency="true"></iframe>
