...
add_action( 'widgets_init', 'solopine_promo_load_widget' );

function solopine_promo_load_widget() {
	register_widget( 'solopine_promo_widget' );
}

class solopine_promo_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_promo_widget', 'description' => __('Display a promo box with an image and link', 'solopine') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_promo_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_promo_widget', __('Rosemary: Promo Box', 'solopine'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'image_url' => '', 'image_link' => '', 'heading' => '', 'border' => false, 'height' => 130, 'margin' => 38);
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>

		<!-- Widget Title: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title', 'solopine' ); ?>:</label>
		...
		<!-- Background URL -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_url' ); ?>"><?php esc_html_e( 'Background Image URL', 'solopine' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the background image URL you want to use. You can upload your background image via Media > Add New', 'solopine' ); ?></small>
		</p>
		
		<!-- Image Text -->
		<p>
			<label for="<?php echo $this->get_field_id( 'heading' ); ?>"><?php esc_html_e( 'Promo Text', 'solopine' ); ?>:</label>
		...
		<!-- Link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_link' ); ?>"><?php esc_html_e( 'Promo Link', 'solopine' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter a link you want the promo box to go to.', 'solopine' ); ?></small>
		</p>
		
		<!-- Height -->
		<p>
			<label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php esc_html_e( 'Promo Box Height', 'solopine' ); ?>:</label><br>
		...
		<!-- Margin -->
		<p>
			<label for="<?php echo $this->get_field_id( 'margin' ); ?>"><?php esc_html_e( 'Promo Box Margin Bottom', 'solopine' ); ?>:</label><br>
		...
		<!-- New tab -->
		<p>
			<label for="<?php echo $this->get_field_id( 'newtab' ); ?>"><?php esc_html_e( 'Open link in a new tab?', 'solopine' ); ?></label>
